...
# keep all rows where Origin = MSY 
...
# only keep rows where Origin=MSY
data2007 <- subset(data2007, Origin=="MSY") 
data2006 <- subset(data2006, Origin=="MSY") 
data2005 <- subset(data2005, Origin=="MSY") 
data2004 <- subset(data2004, Origin=="MSY") 

# SELECT ROWS WHERE ORIGIN=MSY
...
write.csv(data,paste(getwd(),"/dataset/msy_origin.csv",sep="") ,col.names=TRUE, na="",sep=",")

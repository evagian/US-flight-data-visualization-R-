...
data = read.csv(paste(getwd(),"/dataset/dest_msy_data_wdate.csv",sep="") , header=T)
...
ggplot(df, aes(x=interaction(ye), y=delayreas.mean, fill=delayreas.var)) +   
  geom_bar(position='stack', stat='identity') + xlab("Year") + ylab("Mean Time of Delay (minutes)") + ggtitle("Flights Arriving To MSY")+ scale_fill_discrete(name = "Reason of Delay")
